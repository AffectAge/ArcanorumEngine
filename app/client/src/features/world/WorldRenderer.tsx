...
export function WorldRenderer({ world, ariaLabel, failureLabel, onHexSelect }: WorldRendererProps) {
  ...
  useEffect(() => {
    ...
    setError(undefined);
    const game = createWorldGame(parent, world, (message) => setError(message), onHexSelect);
...
function createWorldGame(
  ...
  onHexSelect: (hex: WorldHex) => void,
): Phaser.Game {
  return new Phaser.Game({
    type: Phaser.AUTO,
    parent,
    backgroundColor: '#000000',
    scene: [new WorldScene(world, onFailure, onHexSelect)],
    input: {
      activePointers: 2,
    },
    scale: {
...
class WorldScene extends Phaser.Scene {
  ...
  private onPointerUp(pointer: Phaser.Input.Pointer): void {
    ...
    const shouldSelectHex =
      !wasPinching &&
      this.dragPointerId === pointer.id &&
      this.dragOrigin !== undefined &&
      pointerDistanceFromOrigin(pointer, this.dragOrigin) <= 8;
    ...
    if (shouldSelectHex) {
      this.selectHexAtPointer(pointer);
    }
  ...
  private selectHexAtPointer(pointer: Phaser.Input.Pointer): void {
    pointer.updateWorldPoint(this.cameras.main);
    const hex = findHexAtWorldPosition(this.world, pointer.worldX, pointer.worldY);
    if (hex === undefined) {
      return;
    }
...
function pointerDistanceFromOrigin(
  pointer: Phaser.Input.Pointer,
  origin: { readonly x: number; readonly y: number },
): number {
  return Math.hypot(pointer.x - origin.x, pointer.y - origin.y);
}
